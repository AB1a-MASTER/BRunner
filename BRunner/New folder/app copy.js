...
// ============================================================================
// SIDEBAR UX INTERACTION: Notify the extension that the Studio is active
// ============================================================================
chrome.runtime.sendMessage({ type: "STUDIO_LOADED" });
...
function addStepToWorkflow(actionType) {
  ...
  workflowSteps.push({
    ...
    action: actionType,
    target: "",
    payload: actionType === "browser.navigate" ? { openIn: "sameTab" } : {},
    openIn: actionType === "browser.navigate" ? "sameTab" : undefined,
  });
  renderCanvas();
}
    ...
    switch (step.action) {
      case "browser.navigate":
        ...
        payloadHtml = `<div class="node-input-group"><label>URL</label><input type="text" id="payload1-${step.id}" value="${step.payload.primary || ""}" placeholder="https://google.com"></div>
          <div class="node-input-group"><label>Open In</label><select id="openin-${step.id}">
