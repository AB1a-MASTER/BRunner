export function projectRuntimeState(
  ...
  const unresolved = new Set(execution.unresolvedNodeIds || []);
  const active = ["starting", "running", "cancelling"].includes(execution.status);

  return nodes.map((node) => {
    let runtimeStatus = "idle";
    if (unresolved.has(node.id)) runtimeStatus = "unresolved";
    else if (skipped.has(node.id)) runtimeStatus = "skipped";
...
export function summarizeExecution(execution = {}) {
  ...
  const unresolved = execution.unresolvedNodeIds?.length || 0;
  switch (execution.status) {
    ...
    case "completed": return `Completed ${completed} · bypassed ${skipped} · unresolved ${unresolved}`;
